...
namespace dxvk {
  ...
  Rc<DxvkSampler> DxvkUnboundResources::createSampler(DxvkDevice* dev) {
    ...
    info.setAddressModes(VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE);
    info.setLodRange(-256.0f, 256.0f, 0);
    info.setAniso(0);
    info.setDepthCompare(VK_FALSE, VK_COMPARE_OP_NEVER);
    info.setReduction(VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE);
    info.setBorderColor(VkClearColorValue());
    info.setUsePixelCoordinates(VK_FALSE);
  ...
  Rc<DxvkImage> DxvkUnboundResources::createImage(
    ...
    auto s = "Null Image-" + std::to_string((int)type + 1);
    info.debugName   = s.c_str();
  ...
  void DxvkUnboundResources::clearImage(
    const Rc<DxvkContext>&  ctx,
    const Rc<DxvkImage>&    image) {
    ctx->initImage(image, VK_IMAGE_LAYOUT_UNDEFINED);
  }
